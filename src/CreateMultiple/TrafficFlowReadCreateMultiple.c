...
int main()
{
    ...
    /* PROGRAM VARIABLES */
    short i=0, j=0, multiFile=0;        // loop counters
    /// CONSTANTS
    ...
    // Time
        ...
        float dt = .1;
    /* VARIABLES DEAL WITH MULTIPLE FILE MANAGEMENT */
    char fileName[100];
    ...
    for( multiFile=0; multiFile<finstruct_num; multiFile++ ){
        ...
        printf("Creating Output %d/%d for File: %s\n", multiFile,finstruct_num,fileName);
